...
public class DefaultRemoteCommand implements RemoteCommand {
	// as we have beginning and ending pipes, we will have 1 more entry than we
	// need
	...
	public DefaultRemoteCommand(String command, String[] args) {
		...
		if ("selectWindow".equals(command) && args[0] == null) {
			// hackylicious I know, but what a dorky interface! Users naturally
			// give us too much credit, and submit a null argument
			// instead of a string "null". Our code elsewhere assumes that all
			// arguments are non-null, so
	...
	public String getCommandURLString() {
		...
		for (int i = 0; i < args.length; i++) {
			...
			sb.append('=');
			sb.append(command == "getNewBrowserSession" && i == 0 ? args[i].replace("\\", "\\\\") : urlEncode(args[i]));
		}
		return sb.toString();
	}
	...
	public static RemoteCommand parse(String inputLine) {
		...
		if (values.length != NUMARGSINCLUDINGBOUNDARIES) {
			throw new IllegalStateException("Cannot parse invalid line: "
					+ inputLine + values.length);
		}
		return new DefaultRemoteCommand(values[FIRSTINDEX], new String[] {
				values[SECONDINDEX], values[THIRDINDEX] });
	}
