...
public class HttpCommandProcessor implements CommandProcessor {
	...
	protected String pathToServlet;
	protected String browserStartCommand;
	protected String browserURL;
	protected String sessionId;

	/**
	 * Specifies a server host/port, a command to launch the browser, and a
	 * starting URL for the browser.
	 * 
	 * @param serverHost
	 ...
	 * @param browserURL
	 *            - the starting URL including just a domain name. We'll start
	 *            the browser pointing at the Selenium resources on this URL,
	 *            e.g. "http://www.google.com" would send the browser to
	 *            "http://www.google.com/selenium-server/core/RemoteRunner.html"
	 */
	public HttpCommandProcessor(String serverHost, int serverPort,
	...
	/**
	 * Specifies the URL to the CommandBridge servlet, a command to launch the
	 * browser, and a starting URL for the browser.
	 * 
	 * @param pathToServlet
	 *            - the URL of the Selenium Server Driver, e.g.
	 *            "http://localhost:4444/selenium-server/driver/" (don't forget
	 *            the final slash!)
	 * @param browserStartCommand
	 *            - the command string used to launch the browser, e.g.
	 *            "*firefox" or
	...
	public String executeCommandOnServlet(String command) {
		...
		try {
			...
			throw new UnsupportedOperationException(
					"Catch body broken: IOException from " + command + " -> "
							+ e);
		} finally {
			if (is != null) {
				try {
	...
	protected String stringContentsOfInputStream(InputStream is)
			throws IOException {
	...
	protected InputStream getCommandResponse(String command, InputStream is)
			throws IOException {
	...
	protected String buildCommandBody(String command)
	...
	public void start() {
		String result = getString("getNewBrowserSession", new String[] {
	...
	public String getString(String commandName, String[] args) {
		...
		System.err.println("WARNING: getString(" + commandName
				+ ") saw a bad result " + result);
		return "";
	}
	...
	/**
	 * Convert backslash-escaped comma-delimited string into String array. As
	 * described in SRC-CDP spec section 5.2.1.2, these strings are
	 * comma-delimited, but commas can be escaped with a backslash "\".
	 * Backslashes can also be escaped as a double-backslash.
	 * 
	 * @param input
	 *            the unparsed string, e.g. "veni\, vidi\,
	 *            vici,c:\\foo\\bar,c:\\I came\, I \\saw\\\, I conquered"
